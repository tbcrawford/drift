...
| `go install`-able CLI, zero external deps at runtime | Go's toolchain makes this easy but most diff tools (delta: Rust binary, diff-so-fancy: Perl+npm) require system package managers. | LOW | `go install github.com/tbcrawford/drift/cmd/drift@latest` — single static binary, nothing to install. |
